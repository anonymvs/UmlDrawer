...
void DiagramReader::readFromPath(std::string file_name) {
    std::ifstream is;
...
std::map<std::string, std::string> DiagramReader::parseParams(std::string const &params) {
    ...
    for(auto v : tmp) {
        ...
        if(param.size() == 3) {
            //ret.find(param[1])->second = param[2];
            ret.insert(std::pair<std::string, std::string>{param[1], param[2]});
            //ret[param[1]] = param[2];
        } else {
            ret.insert(std::pair<std::string, std::string>{param[0], param[1]});
            //ret.find(param[0])->second = param[1];
